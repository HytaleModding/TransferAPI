...
import com.shailist.TransferAPI.api.transfer.v1.storage.base.CombinedStorage;
import com.shailist.TransferAPI.api.transfer.v1.storage.base.ExtractionOnlyStorage;
import com.shailist.TransferAPI.api.transfer.v1.storage.base.InsertionOnlyStorage;
 ...
 *     <li>Resource-specific base implementations may also be available.</li>
 * </ul>
 *
...
public interface Storage<T> extends Iterable<StorageView<T>> {
	 ...
	 * Return false if calling {@link #insert} will absolutely always return 0, or true otherwise or in doubt.
	 *
	 * @return {@code true} if insertion may be supported, {@code false} if it is definitely unsupported
	 */
	 ...
	 * Return false if calling {@link #extract} will absolutely always return 0, or true otherwise or in doubt.
	 *
	 * @return {@code true} if extraction may be supported, {@code false} if it is definitely unsupported
	 */
	 ...
	 */
	/**
	 * Same as {@link #iterator()} but guaranteed to skip over empty views.
	 *
	 * @return iterator over non-empty storage views
	 */
	default Iterator<StorageView<T>> nonEmptyIterator() {
		return java.util.stream.StreamSupport.stream(((Iterable<StorageView<T>>) this::iterator).spliterator(), false)
				.filter(view -> view.getAmount() > 0 && !view.isResourceBlank())
				.iterator();
	}
	 ...
	 * }</pre>
	 *
	 * @return an {@link Iterable} that iterates over non-empty {@link StorageView}s from this storage
	 */
	 ...
	 *
	 * @return a non-negative version id that changes when the storage's state may have changed
	 */
	...
	/**
	 * Return a {@code Class} instance representing this {@code Storage} interface specialized for a type.
	 *
	 * <p>This helper is useful for APIs that accept {@code Class<Storage<T>>} tokens to register or look up
	 * storage providers for a particular resource type.
	 *
	 * @param <T> the resource type parameter for the returned class token
	 * @return a {@code Class} object representing {@code Storage<T>}
	 */
	@SuppressWarnings("unchecked")
